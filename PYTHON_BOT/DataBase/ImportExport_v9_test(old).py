...
class Map:
    def add(message,user,bot): #commit()
            try:
                ...
                orgid = 0     
            cnxn = pypyodbc.connect('DRIVER={SQL Server};SERVER=a380;DATABASE=smap;UID=sc;PWD=masterkey')
            cursor = cnxn.cursor()
            if user.tz == 0:
                inv = user.inv
                name = user.name_skz
...
class Naryad:
    ...
    def add_to_ehz(message,user,bot):#con.commit()
            ...
            try:
                ...
                bot.reply_to(message, "Данные по объекту обновлены!" + str(errorMsg))
            con.close()
    ...
    def get_tz_info(message,user):
        ...
        if TZ_id is not None:
            ...
            for row in cur.itermap():
                ...
                user.adres = str(str(row['Nas_Punkt'])+", "+str(row['Vul'])+", "+ str(row['Dom']))
    ...
    def get_tz(message,user):
        ...
        my_array = []
        for row in cur.itermap():
            name_for_user = str(row['type_name'])+" "+str(row['TZ_name'].title())
    ...
    def get_skz(message,user):
            ...
            " group by SKZ_id,SKZ_Name,Naryad_Key"+
            " Order by SKZ_id ASC")
            
            my_array = []
            for row in cur.itermap():
                my_array.append(str(row['SKZ_Name'])+" ; "+str(row['SKZ_id'])+" ; 170")
            cur.close()
            con.close()
            return my_array
        ...
        try:
            ...
            try:
                ...
                cur.execute("select t_counter.F354 as nas_punkt,"+
                        ...
                        " max(t_pokaz.F379) as coefficient,"+
                        " max(t_pokaz.F369) as lastpokaz,"+
